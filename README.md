...
Last N turns of a triage call

Scratchpad

Current goal / sub-goal / working state, isolated from the buffer so pruning never destroys it

"Review open rate exceptions before approving shipment 512" survives even after the tool-call chatter that gathers each exception gets pruned

PromoteDropRouter

Decides forget vs. episodic for each turn evicted from short-term memory, with a logged reason. Never writes to semantic memory.

A credit hold placement is promoted; "good morning" is forgotten

EpisodicMemory

Durable, queryable-by-customer store of promoted events

"Credit hold placed on customer 12, severe, 90+ days overdue"

ConsolidationLayer

Separate, periodic pass over episodic memory that derives/updates semantic facts (never triggered inline by the router)

Turns repeated credit-hold episodes into a current customer_risk_level fact

SemanticMemory

Versioned, expiring facts with explicit conflict resolution

See conflict example below

A real conflict, resolved

Customer 12's credit hold is released → consolidation writes
customer_risk_level = good_standing (version 1, active).

Weeks later, a new severe credit hold lands on the same customer
(90+ days overdue) → the next consolidation pass detects that this
contradicts the active fact.

Resolution: version 1 is marked superseded (not deleted) and
...
fact_history() still returns both versions, so the full timeline
...
The client reads the server's declared capabilities and gates protocol operations on them

Notifications

Authentication can change the exposed tool set and triggers tools/list_changed

Elicitation

Above-authority discounts and severe credit-hold releases require explicit human input

Sampling

run_portfolio_risk_sweep can request a narrative summary from the connected client model

Resources

Credit/discount authority policy is exposed as an MCP resource

Prompts

Parameterized rate-exception justification prompt

Transport

stdio for local development and Streamable HTTP for remote execution

Progress tracking

Portfolio risk sweep reports progress while customers are processed

Defensive tools

Strict Pydantic schemas, server-side validation, role/state checks, safe failures, and re-authorization before writes
...
Mask older tool outputs while preserving the conversation

Zone-Based Pruning

Preserve system/important zones and recent context
...
Section-aware, max 1000 characters, 120-character overlap

Embeddings

BAAI/bge-small-en-v1.5 with FastEmbed
...
Role, status, document, and section metadata applied during retrieval
...
check retrieved evidence for relevance;

generate only from the retrieved evidence;

validate citations and factual support;

check numeric claims against evidence or scenario values;

return a safe abstention when verification fails.
